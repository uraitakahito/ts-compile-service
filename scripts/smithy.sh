...
#
# 生成物 (generated/ssdk と generated/openapi.json) は commit する。読む人が JVM 無しで読めるため、
# そして runtime の image に codegen を持ち込まないため。CI は同じ script を走らせて
# `git diff --exit-code -- generated` を見る (pnpm run smithy:check) —— model と生成物がずれていれば赤。
#
# `generated/ssdk` は毎回消して作り直す。model から消えた operation の生成物が残らないように。
# `generated/package.json` (type: commonjs) と `generated/tsconfig.json` は ssdk の外に在るので残る。
set -euo pipefail
...
case "${1:-build}" in
  ...
  build)
    ...
    "$CLI" build --quiet
    rm -rf generated/ssdk
    mkdir -p generated/ssdk
    ...
    echo "generated/ssdk と generated/openapi.json を書き直した" >&2
    ;;
  *)
